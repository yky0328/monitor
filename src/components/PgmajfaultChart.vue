<template>
  <div id="pgmajfault" style="width: 1000px; height: 400px"></div>
</template>
...
export default {
  ...
  data() {
    return {
      myChart: null,
      query: 'rate(node_vmstat_pgmajfault{instance="c4"}[1m])',
      monitor_data: null,
    };
  },
  watch: {
  ...
  mounted() {
    this.myChart = echarts.init(document.getElementById("pgmajfault"));
    this.drawChart([[]]);
  },
  methods: {
    async confirm() {
    ...
    drawChart(tmpdata) {
      // const chartDom = document.getElementById("pgmajfault");
      // const myChart = echarts.init(chartDom);
      const option = {
        title: {
          text: "node_vmstat_pgmajfault",
        },
        tooltip: {
          trigger: "axis",
          axisPointer: {
