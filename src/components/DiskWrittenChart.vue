<template>
  <div id="diskwritten" style="width: 1000px;height:400px;"></div>
</template>
...
export default {
  ...
  data() {
    return {
      myChart: null,
      query_a:
        'rate(node_disk_written_bytes_total{device="vda", instance="c4"}[1m])',
      query_b:'rate(node_disk_written_bytes_total{device="vdb", instance="c4"}[1m])',
      monitor_data_a: null,
      monitor_data_b: null,
    };
  },
  ...
  mounted() {
    this.myChart = echarts.init(document.getElementById("diskwritten"));
    this.drawChart([[]],[[]]);
  },
  methods: {
    ...
    drawChart(tmpdata1, tmpdata2) {
      // const chartDom = document.getElementById('diskwritten');
      // const myChart = echarts.init(chartDom);
      const option = {
        title: {
          text: 'node_disk_written_bytes_total'
        },
        tooltip: {
          trigger: 'axis',
          axisPointer: {
        ...
        series: [
          {
            name: 'disk_written_device=vda',
            type: 'line',
            data: [] // Y轴数值数据将在下面填充
          },
          {
            name: 'disk_written_device=vdb',
            type: 'line',
