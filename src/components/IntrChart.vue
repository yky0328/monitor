<template>
  <div id="intrchart" style="width: 1000px; height: 400px"></div>
</template>
...
export default {
  ...
  data() {
    return {
      myChart: null,
      query:
        'rate(node_intr_total{instance="c4"}[1m])',
      monitor_data: null,
    };
  },
  watch: {
  ...
  mounted() {
    this.myChart = echarts.init(document.getElementById("intrchart"));
    this.drawChart([[]]);
  },
  methods: {
    async confirm() {
    ...
    drawChart(tmpdata) {
      // const chartDom = document.getElementById("intrchart");
      // const myChart = echarts.init(chartDom);
      const option = {
        title: {
          text: "node_intr_total",
        },
        tooltip: {
          trigger: "axis",
          axisPointer: {
