<template>
  <div id="pgfault" style="width: 1000px; height: 400px"></div>
</template>
...
export default {
  ...
  data() {
    return {
      myChart: null,
      query:
        'rate(node_vmstat_pgfault{instance="c4"}[1m])',
      monitor_data: null,
    };
  },
  watch: {
  ...
  mounted() {
    this.myChart = echarts.init(document.getElementById("pgfault"));
    this.drawChart([[]]);
  },
  methods: {
    ...
    drawChart(tmpdata) {
      // const chartDom = document.getElementById("pgfault");
      // const myChart = echarts.init(chartDom);
      const option = {
        title: {
          text: "node_vmstat_pgfault",
        },
        tooltip: {
          trigger: "axis",
          axisPointer: {
