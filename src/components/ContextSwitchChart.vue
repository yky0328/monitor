<template>
  <div id="cs" style="width: 1000px; height: 400px"></div>
</template>
...
export default {
  ...
  data() {
    return {
      myChart: null,
      query:
        'rate(node_context_switches_total{instance="c4"}[1m])',
      monitor_data: null,
    };
  },
  watch: {
    starttime: async function (val) {
      console.log("starttime changed", val);
      await this.confirm();
      console.log("qushuju");
    ...
    endtime: async function (val) {
      console.log("endtime changed", val);
      await this.confirm();
      console.log("qushuju");
      console.log(this.monitor_data);
      this.drawChart(this.monitor_data);
  ...
  mounted() {
    this.myChart = echarts.init(document.getElementById("cs"));
    this.drawChart([[]]);
  },
  methods: {
    async confirm() {
    ...
    drawChart(tmpdata) {
      // const chartDom = document.getElementById("cs");
      // const myChart = echarts.init(chartDom);
      const option = {
        title: {
          text: "node_context_switches_total",
        },
        tooltip: {
          trigger: "axis",
          axisPointer: {
